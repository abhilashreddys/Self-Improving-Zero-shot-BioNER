...
def parse_args():
    ...
    ##=== input data ===##
    parser.add_argument("--model_name", default='meta-llama/Llama-3.2-3B-Instruct', type=str)
    ...
    while not len(unique_result):
        ...
        for rst in result:
            ...
            if not (n_code_steps * len(_probs)) or n_code_steps > len(_probs):
                continue
            lines, probs = split_code_with_probs(code, _probs, not_code=(args.prompts['type'] in ['commonsense', 'ner']))
...
def generate_code_beam_search(args, exp):
    ...
    if args.prompts['type'] not in ['commonsense', 'ner']:
    ...
    for idx, ins in enumerate(instances):
        ...
        rest_code = []
        if not finished or (not ins[-1].startswith('    return ') and args.prompts['type'] not in ['commonsense', 'ner']):
